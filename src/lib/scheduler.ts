 ...
 *
 * Key features:
 * 1. Week starts on Saturday (Egyptian work week)
 * 2. One employee OFF per week (all 7 days, Sat-Fri)
 ...
 * 6. Multi-factor scoring for OFF selection and daily assignments
 * 7. Post-generation swap optimization to reduce hour variance
 * 8. Cumulative stats carry forward across months for long-term balance
 */
...
function getWeeksInMonth(year: number, month: number): Date[][] {
  ...
  // Find the first Saturday on or before the first day
...
export function generateScheduleForMonth(
  ...
  for (let wn = 0; wn < weeks.length; wn++) {
    ...
    // Track this person as recently off
    recentlyOff.add(offPerson);

    // Remove from recentlyOff if enough time has passed (keep only last 1)
    // Actually, we keep the last off person to prevent consecutive
  }
